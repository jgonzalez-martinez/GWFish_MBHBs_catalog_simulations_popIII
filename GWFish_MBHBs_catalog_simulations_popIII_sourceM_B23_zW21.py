...
from catalog_MBHBs_popIII_sourceM_B23_zW21 import N_drawn, generate_synthetic_MBHB_population 
...
pop = generate_synthetic_MBHB_population(
    ...
    q_min=0.9, q_break=0.88, q_max=1.0, 
...
#df.to_csv("mbhb_popIII_catalog_B23_zW21.tsv", sep="\t", index=False)


# In[5]:


# We choose a waveform approximant
...
#df = pd.DataFrame(snr)
#df.to_csv("mbhb_popIII_catalog_snr_B23_zW21.tsv", sep="\t", index=False)
...
gw.fishermatrix.analyze_and_save_to_txt(network = network,
                                        ...
                                        population_name = f'MBHB_catalog_popIII_withfisher_tilts_sourceM_B23_zW21_PhenomPv2',
                                        waveform_model = waveform_model,
                                        f_ref = 1e-4,
